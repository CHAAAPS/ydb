...
map<timestamp, string> TIMESTAMP_NAMES = {
    ...
    {timestamp::STRINGS_EXCHANGE_COUNT, "String Exchange count"},
    {timestamp::NETWORK_DELAY, "Network Delay"}};
...
// vector<char *> read_data_from_vector(vector<char *> &strings, int start, int end)
// {
//     // vector<char *> res;
//     // for (size_t i = start; i < end; i++){
//     //     res.push_back(strdup(res[i]));
//     // }
//     return vector<char *>(strings.begin() + start, strings.begin() + end);
// }

void mergeSequences(
    std::vector<std::pair<std::vector<char *>::iterator, std::vector<char *>::iterator>> &sequences,
    std::vector<char *> &sorted_strings)
{

    auto cmp = [](std::pair<char *, size_t> a, std::pair<char *, size_t> b)
    {
        return std::strcmp(a.first, b.first) > 0;
    };
    std::priority_queue<std::pair<char *, size_t>, std::vector<std::pair<char *, size_t>>, decltype(cmp)> minHeap(cmp);

    for (size_t i = 0; i < sequences.size(); ++i)
    {
        if (sequences[i].first != sequences[i].second)
        {
            minHeap.emplace(*sequences[i].first, i);
        }
    }

    std::vector<std::vector<char *>::iterator> iterators;
    for (auto &seq : sequences)
    {
        iterators.push_back(seq.first);
    }

    while (!minHeap.empty())
    {
        auto [smallest, seqIndex] = minHeap.top();
        minHeap.pop();

        sorted_strings.push_back(smallest);

        if (++iterators[seqIndex] != sequences[seqIndex].second)
        {
            minHeap.emplace(*iterators[seqIndex], seqIndex);
        }
    }
}

double emulateBandwidth(size_t dataSizeBytes, size_t bandwidthMbps)
{
    if (bandwidthMbps == 0)
        return 0;
    double bandwidthBytesPerSec = (bandwidthMbps * 1024 * 1024) / 8;
    double transmissionTimeSec = dataSizeBytes / bandwidthBytesPerSec;
    auto sleepTime = std::chrono::duration<double>(transmissionTimeSec);
    auto transmissionTimeMs = transmissionTimeSec * 1000;
    // std::this_thread::sleep_for(sleepTime);
    cout << "sleeping for: " << transmissionTimeMs << endl;
...
void fill_data()
{
    ...
    // size_t strings_num = 4'000'000;
    size_t strings_num = 800'000;
    vector<size_t> STR_LENGTHS = {20};
...
void add_timestamp_in_ms(map<timestamp, vector<double>> &execution_timestamps, timestamp timestamp_name, size_t execution_time, size_t id)
{
    execution_timestamps[timestamp_name][id] += execution_time;
}
                       ...
                       double time_ms, double speed)
{
    ...
    // no_delay
    // all
    std::ofstream result("results/2712/" + std::to_string(num_nodes) + "_nodes_" + std::to_string(p / num_nodes) + "_factor_" + static_cast<string>(FILENAME));
    result << "Single Thread Sorting Time: " << time_ms << "[ms]" << std::endl;
    result << "Single Thread Speed: " << speed << "[MB/s]\n"
           << endl;

    // Determine the number of rows and columnsgit
    int rows = data.size();
    int cols = 0;
    ...
    row_name = "Speed, MB/s";
    result << endl
           << row_name << setw(first_col_size - row_name.size()) << "|";
